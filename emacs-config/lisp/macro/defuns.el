...
(defun user-zap-to-char (arg char)
...
(defun user-ido-find-config ()
  (interactive)
  (find-file
   (concat "~/.emacs.d/macro/"
           (ido-completing-read "Config file: "
                                (directory-files "~/.emacs.d/macro/"
                                                 nil
                                                 "^[^.]")))))

(defun user-delete-till-end-of-buffer ()
...
(defun user-ido-find-project ()
  (interactive)
  (find-file
   (concat "~/Code/" (ido-completing-read "Project: "
                           (directory-files "~/Code/" nil "^[^.]")))))

(defun user-goto-config ()
  (interactive)
  (find-file "~/.emacs.d/macro.el"))

;; fix kill-word
(defun user-kill-word (arg)
  ...
  (let ((whitespace-regexp "\\s-+"))
    (kill-region (point)
                 (cond
                  ...
                  ((looking-at "\n") (kill-line) (user-kill-word arg))
                  (t (forward-word arg) (point))))))

(defun user-backward-kill-word (arg)
...
(defun user-shebang-to-mode ()
  ...
  (let*
    ...
    (when (functionp mode-fn)
      (funcall mode-fn))))
;(add-hook 'find-file-hook 'user-shebang-to-mode)

; duplicate the current line
(defun user-duplicate-line ()
  (interactive)
    (beginning-of-line)
...
(defun vendor (library)
  (let* ((file (symbol-name library))
         ...
         (user (concat "~/.emacs.d/macro/" file)))
    (cond
     ((file-directory-p normal) (add-to-list 'load-path normal) (require library))
     ((file-directory-p suffix) (add-to-list 'load-path suffix) (require library))
     ((file-exists-p suffix) (require library)))
    (when (file-exists-p (concat user ".el"))
      (load user))))

(defun user-backward-kill-line ()
  (interactive)
  (kill-line 0))

(require 'thingatpt)
(defun user-change-num-at-point (fn)
...
(defun user-inc-num-at-point ()
  (interactive)
  (user-change-num-at-point '+))

(defun user-dec-num-at-point ()
  (interactive)
  (user-change-num-at-point '-))
...
(defun gist-buffer-confirm (&optional private)
  ...
  (defun user/c-electric-brace (arg)
